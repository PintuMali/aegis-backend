...
impl DynamoRepository {
    ...
    pub async fn query_gsi1(&self, gsi1_pk: &str, gsi1_sk_prefix: Option<&str>) -> Result<Vec<GameEntity>> {
        ...
        expression_values.insert(":gsi1_pk".to_string(), AttributeValue::S(gsi1_pk.to_string()));

        if let Some(prefix) = gsi1_sk_prefix {
            query = query.key_condition_expression("gsi1_pk = :gsi1_pk AND begins_with(gsi1_sk, :prefix)");
